(function () {
    ...
    return `
      <aside class="sidebar glass-panel">
        ...
        <nav class="sidebar-nav">
          ...
          <a class="nav-link ${route.name === "favorites" ? "is-active" : ""}" href="#/favoritos">
            <span>Favoritos</span>
  ...
  function renderHomeView(currentState, route) {
    const isFavoritesRoute = route && route.name === "favorites";
    const prompts = isFavoritesRoute
      ? currentState.prompts.filter((prompt) => currentState.user.favorites.includes(prompt.id))
      : currentState.prompts;

    if (isFavoritesRoute && !prompts.length) {
      return {
        html: `
          <section class="empty-panel glass-panel">
            <span class="eyebrow">Favoritos</span>
            <h2 class="section-title">TodavÃ­a no guardaste prompts en favoritos.</h2>
            <p>Marca prompts con la estrella y aparecerÃ¡n reunidos en esta vista.</p>
            <div class="hero-actions">
              <a class="primary-button" href="#/">Explorar prompts</a>
            </div>
          </section>
        `,
        bind: function bindEmptyFavorites() {
          return function noop() {};
        },
      };
    }
    ...
    return {
      html: `
        ...
        <section class="toolbar glass-panel">
          <div>
            <span class="eyebrow">ColecciÃ³n personal</span>
            <h2 class="section-title">Tus prompts favoritos</h2>
          </div>
        </section>
        ` : ""}
        <section class="toolbar glass-panel">
  ...
  function renderDetailView(currentState, promptId) {
    ...
    return {
      ...
      bind: function bindDetail(context) {
        const unbindCarousel = bindCarousel(document.querySelector("[data-carousel]"));
        window.setTimeout(() => {
          context.ensurePromptView(prompt.id);
        }, 0);
        return unbindCarousel;
      },
    };
  }
    ...
    return {
      ...
      bind: function bindAdmin(context) {
        ...
        function submitHandler(event) {
          ...
          if (adminDraft.id) {
            const editingPromptId = adminDraft.id;
            adminFlashMessage = `Prompt "${payload.title}" actualizado correctamente.`;
            context.actions.updatePrompt(editingPromptId, payload);
            hydrateDraft(null);
    ...
    const view =
      route.name === "detail"
        ...
        : route.name === "admin"
          ...
          : renderHomeView(currentState, route);
